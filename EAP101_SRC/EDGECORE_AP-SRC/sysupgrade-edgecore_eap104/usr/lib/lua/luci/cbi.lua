...
function Map.parse(self, readinput, ...)
	...
	self:_run_hooks("on_after_parse")
...
-- accton: Allow to add dendency on another section (section2, optional)
function AbstractValue.depends(self, field, value, section2)
	local deps
...
function AbstractValue.parse(self, section, novld)
	...
	if fvalue and #fvalue > 0 then -- If we have a form value, write it to UCI
		...
		if not fvalue and not novld then
			...
			luci.util.perror("[LUCI][ERROR] One or more fields contain invalid values.")
			luci.util.perror("[LUCI][ERROR]   " .. tostring(section) .. "." .. tostring(self.option) .. "")
		end
