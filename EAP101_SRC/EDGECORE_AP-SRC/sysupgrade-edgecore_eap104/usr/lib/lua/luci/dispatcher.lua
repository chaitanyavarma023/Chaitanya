...
function show_ez_wizard()
	require("luci.template")

	local domain = luci.http.getenv("HTTP_HOST") or "n/a"
	context.path = {}
	luci.template.render("admin_system/easy_wizard",  { domain=domain })
	
	return false
end

function show_qr_wizard()
	require("luci.template")

	local domain = luci.http.getenv("HTTP_HOST") or "n/a"
	context.path = {}
	luci.template.render("admin_system/qr_wizard",  { domain=domain })
...
function dispatch(request)
  ...
  -- https://redmine.ignitenet.com/issues/26437
  -- /tmp/luci-indexcache's size should not be 0.
  if indexcache then
		local cachesize = fs.stat(indexcache, "size")
		if cachesize and cachesize <= 0 then
		  os.remove(indexcache)
		end
	end
	...
	if type(auth) == "table" and type(auth.methods) == "table" and #auth.methods > 0 then
		...
		if not (sid and sdat and sacl) and auth.login then
			...
			-- get username
			if user ~= nil then
				luci.http.header("Set-Cookie", "user_name=" .. user .."; path="..build_url())
			end
			...
			if ez_setup == "1" or ez_setup == "3" then
				if build_url(unpack(ctx.requestpath)) == "/cgi-bin/luci/admin/system/qr_wizard" then
					show_qr_wizard()
				else
					show_ez_wizard()
				end
		...
		local ez_setup = uci:get("acn", "wizard", "ez_setup")
		if ez_setup == "1" and build_url(unpack(ctx.requestpath)) == "/cgi-bin/luci/" then
			show_ez_wizard()
			return
		end

		-- fixed https://redmine.ignitenet.com/issues/19318
		if (build_url(unpack(ctx.requestpath)) ~= "/cgi-bin/luci/" and
			build_url(unpack(ctx.requestpath)) ~= "/cgi-bin/luci/admin/system/wizard" and 
			build_url(unpack(ctx.requestpath)) ~= "/cgi-bin/luci/admin/system/qr_wizard" and 
			build_url(unpack(ctx.requestpath)) ~= "/cgi-bin/luci/admin/system/qrwizard" and 
			build_url(unpack(ctx.requestpath)) ~= "/cgi-bin/luci/admin/system/ezwizard" and 
			build_url(unpack(ctx.requestpath)) ~= "/cgi-bin/luci/admin/system/ezwizard_mgmt" and 
			build_url(unpack(ctx.requestpath)) ~= "/cgi-bin/luci/admin/translations/en") and (ez_setup ~= "0") then
	...
	if action.path == "admin_status/index" and controller_mode == "3" then
		action.type = "view"
		action.path = "ucentral/status"
	end
