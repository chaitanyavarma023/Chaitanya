...
function getIpAliasAddressList(){var _ipAliasArray=new Array();_ipAliasArray['Name']=new Array();_ipAliasArray['IP']=new Array();_ipAliasArray['Netmask']=new Array();_ipAliasArray['friendly']=new Array();$('.cbi-value-field input').each(function(input){var data_lan_value=$('#cbi-network').find('div[data-lan="lan"]').attr('data-lan');if(this.id.indexOf('ipaddr')>=0&&this.value!=''){var tr_css_display=($(this).closest('tr').css('display')!='none');if(tr_css_display){_ipAliasArray['IP'].push(this.value);_ipAliasArray['Name'].push(data_lan_value);_ipAliasArray['friendly'].push(network_prefix(data_lan_value));}}
...
return outputArray;}
function validateNetwork(ip_id,netmask_id){var ip_value=$(ip_id).val();var netmask_value=$(netmask_id).val();var res_ip=true,res_netmask=true;if(!is_ip(ip_value)){set_err_msg($(ip_id),true,'Not a valid IPv4 address!');res_ip=false;}
if(!cbi_validators.netmask(netmask_value)){set_err_msg($(netmask_id),true,'Not a valid netmask!');res_netmask=false;}
if(res_ip&&res_netmask){if(is_broadcast(ip_value,netmask_value)){set_err_msg($(ip_id),true,'IP address is a broadcast IP.');return false;}
...
if(res){set_err_msg($customDns,false);}else{set_err_msg($customDns,true,'Please enter a list of no more than 3 IPs of DNS servers, separated by space, comma, semicolon or newline.');}
return res;}
...
var mask_val=$mask.val().split('.');var sumofbits=0;for(var i=0;i<mask_val.length;i++){var tmpvar=parseInt(mask_val[i],10);var bitsfromleft=h_countbitsfromleft(tmpvar);if(isNaN(bitsfromleft)){set_err_msg($mask,true,'Value not allowed!');return false;}
...
function check_range($io_id,start_num,end_num){var res=true;var io_val=$io_id.val();if(is_in_range(io_val,start_num,end_num)){if(end_num>0){set_err_msg($io_id,false,'Allowed values are between '+start_num+' and '+end_num+', inclusive.');}else{set_err_msg($io_id,false,'Value not allowed!');}}else{if(end_num>0){if(end_num===1){set_err_msg($io_id,true,'Allowed values is 1.');}else{set_err_msg($io_id,true,'Allowed values are integers between '+start_num+' and '+end_num+', inclusive.');}}else{set_err_msg($io_id,true,'Value not allowed!');}
res=false;}
return res;}
...
for(var ipentry in ipdata){if(ipdata.hasOwnProperty(ipentry)){if(!ipentry.startsWith(network)&&ipentry.indexOf('-alias')<0){var _ipentry=ipdata[ipentry];if(ip_value===_ipentry.ip){set_err_msg($(ip_id),true,'Your Internet IP address is the same as the '+_ipentry.friendly+' IP address!');res=false;}
if(is_in_netmask(_ipentry.ip,_ipentry.nm,ip_value)){set_err_msg($(ip_id),true,'Your Internet IP address falls in the same subnet as the '+_ipentry.friendly+' network IP address.');res=false;}}}}
var ipAliasArray=getIpAliasAddressList();for(var i=0;i<ipAliasArray['IP'].length;i++){if(!ipAliasArray['Name'][i].startsWith(network)){if(ip_value===ipAliasArray['IP'][i]){set_err_msg($(ip_id),true,'Your Internet IP address is the same as the '+ipAliasArray['friendly'][i]+' IP address!');res=false;}
if(is_in_netmask(ipAliasArray['IP'][i],ipAliasArray['Netmask'][i],ip_value)){set_err_msg($(ip_id),true,'Your Internet IP address falls in the same subnet as the '+ipAliasArray['friendly'][i]+' IP address.');res=false;}}}});return res;}
...
clear_validate_error(ip_id);clear_validate_error(netmask_id);if(!is_ip(ip_value)){set_err_msg($(ip_id),true,'Not a valid IPv4 address!');res_ip=false;this_res=false;}
if(!cbi_validators.netmask(netmask_value)){set_err_msg($(netmask_id),true,'Not a valid netmask!');res_netmask=false;this_res=false;}
if(res_ip&&res_netmask){if(is_broadcast(ip_value,netmask_value)){set_err_msg($(ip_id),true,'IP address is a broadcast IP.');this_res=false;}
if(is_net(ip_value,netmask_value)){set_err_msg($(ip_id),true,'Not a valid network.');this_res=false;}
for(var ipentry in ipdata){if(ipdata.hasOwnProperty(ipentry)){if(!ipentry.startsWith(network)&&ipentry.indexOf('-alias')<0){var _ipentry=ipdata[ipentry];if(ip_value===_ipentry.ip){set_err_msg($(ip_id),true,'Your Internet IP address is the same as the '+_ipentry.friendly+' IP address!');res_ip=false;this_res=false;}
if(is_in_netmask(_ipentry.ip,_ipentry.nm,ip_value)){set_err_msg($(ip_id),true,'Your Internet IP address falls in the same subnet as the '+_ipentry.friendly+' network IP address.');res_ip=false;this_res=false;}}}}}}
