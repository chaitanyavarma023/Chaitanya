...
function add_lan(){var contents=$('#lan_template').html();$('#alert_up_max').hide();$('#add_lan_btn').attr('disabled',false);var new_lan_name='custom_'+next_lan_id();contents=contents.replace(/%CUSTOM%/g,new_lan_name);var new_ids=$('#network\\.new_ids').val();$('#network\\.new_ids').val(new_ids+';'+new_lan_name);$('#lan_list').append(contents);add_validation_handlers.call($('#cbi-network-'+new_lan_name)[0]);add_button_handlers('#'+new_lan_name+'_title','#cbi-network-'+new_lan_name);if(enable_dhcprelay){$('#cbid\\.network\\.'+new_lan_name+'\\.dhcp').attr('checked',false);$('#cbid\\.network\\.'+new_lan_name+'\\.dhcp').prop('disabled',true);var circuit_id_data='#cbid\\.network\\.'+new_lan_name+'\\.circuit_id_data';$(circuit_id_data).val("br-"+new_lan_name);}
else{$('#cbid\\.network\\.'+new_lan_name+'\\.dhcp').attr('checked',true);var circuit_id_data_row='#cbi-network-'+new_lan_name+'-circuit_id_data';$(circuit_id_data_row).hide();}
enable_dhcp(new_lan_name);if(isExceedMaxNum()){$('#alert_up_max').show();$('#add_lan_btn').attr('disabled',true);$('#maxLansNum').html(MAX_CUSTOM_LANS);}}
...
$(function(){$('#cbi').submit(function(){return before_submit();});if(isExceedMaxNum()){$('#alert_up_max').show();$('#add_lan_btn').attr('disabled',true);$('#maxLansNum').html(MAX_CUSTOM_LANS);}
$('#add_lan_btn').click(add_lan);$('.lan_section').each(function()
{var network=$(this).attr('data-lan');if(network!='%CUSTOM%'){if(enable_dhcprelay){$('#cbid\\.network\\.'+network+'\\.dhcp').prop('disabled',true);}
else{var circuit_id_data_row='#cbi-network-'+network+'-circuit_id_data';$(circuit_id_data_row).hide();}
enable_dhcp(network);}})
add_button_handlers();$('.lan_section').each(add_validation_handlers);$('button.ace-tooltip').tooltip();$('button.disabled').click(function(){if(!$(this).hasClass('disabled')){$('.disabled').removeClass('disabled').attr('rel',null);$(this).addClass('disabled').attr('rel','tooltip');}});$('.cbi-section-table-row').each(function(){add_validation_ipaliases_handlers(this.id);});});function add_button_handlers(header_prefix,widget_prefix){if(header_prefix==null){header_prefix='';}
