...
+'                <option id="cbi-firewall-cfgCFG_ID-target-MARK" value="MARK">MARK</option>'
+'                <option id="cbi-firewall-cfgCFG_ID-target-NOTRACK" value="NOTRACK">NOTRACK</option>'
+'            </select>'
+'        </div>'
...
+'                <option id="cbi-firewall-cfgCFG_ID-family-any" value="any" selected="selected">Any</option>'
...
+'                <option id="cbi-firewall-cfgCFG_ID-dest-empty" value="">Any</option>'
+'                zone_select_option'
+'            </select>'
+'        </div>'
...
+'                <option id="cbi-firewall-cfgCFG_ID-proto-all" value="all">Any</option>'
...
function ip_change(_id){var flag_src_ip=true;var flag_dest_ip=true;var id_arr=_id.split('.');var family_id='#'+id_arr[0]+'\\.'+id_arr[1]+'\\.'+id_arr[2]+'\\.family';var family_value=$(family_id).val();var src_ip_id='#'+id_arr[0]+'\\.'+id_arr[1]+'\\.'+id_arr[2]+'\\.src_ip';var dest_ip_id='#'+id_arr[0]+'\\.'+id_arr[1]+'\\.'+id_arr[2]+'\\.dest_ip';var src_ip_value=$(src_ip_id).val();var dest_ip_value=$(dest_ip_id).val();clear_validate_error(src_ip_id);clear_validate_error(dest_ip_id);if(family_value==='any'){if(src_ip_value!==''&&!cbi_validators.firewall_ipaddr(src_ip_value)){set_err_msg($(src_ip_id),true,'Not a valid IP address!');flag_src_ip=false;}
if(dest_ip_value!==''&&!cbi_validators.firewall_ipaddr(dest_ip_value)){set_err_msg($(dest_ip_id),true,'Not a valid IP address!');flag_dest_ip=false;}}else if(family_value==='ipv4'){if(src_ip_value!==''&&!cbi_validators.firewall_ipv4(src_ip_value)){set_err_msg($(src_ip_id),true,'Not a valid IPv4 address!');flag_src_ip=false;}
if(dest_ip_value!==''&&!cbi_validators.firewall_ipv4(dest_ip_value)){set_err_msg($(dest_ip_id),true,'Not a valid IPv4 address!');flag_dest_ip=false;}}else{if(src_ip_value!==''&&!cbi_validators.ip6addr(src_ip_value)){set_err_msg($(src_ip_id),true,'Not a valid IPv6 address!');flag_src_ip=false;}
if(dest_ip_value!==''&&!cbi_validators.ip6addr(dest_ip_value)){set_err_msg($(dest_ip_id),true,'Not a valid IPv6 address!');flag_dest_ip=false;}}
return(flag_src_ip&&flag_dest_ip);}
function setReadOnlyOnSelect(selectList){if(!selectList.length){console.warn('Unable to find specified select list.');return;}
...
$(function(){$('#cbi').submit(function(){if(!validate_form()){$('#form_error_msg').text('Some fields are invalid, please see messages below!');$('#form_error_msg_placeholder').removeClass('hide');window.document.body.scrollTop=0;window.document.documentElement.scrollTop=0;return false;}else{return true;}});$('#alert_up_max').hide();if(getVisibleRowCount(-1)>=max_count){$('#alert_up_max').show();$('#btn_add').attr('disabled',true);}
...
if(this.id.indexOf('family')>=0){var id_arr=this.id.split('.');var family_id='#'+id_arr[0]+'\\.'+id_arr[1]+'\\.'+id_arr[2]+'\\.'+id_arr[3];$(family_id).change(function(){ip_change(this.id);});ip_change(this.id);}});$('button.ace-tooltip').tooltip();$('button.disabled').click(function(e){if(!$(this).hasClass('disabled')){$('.disabled').removeClass('disabled').attr('rel',null);$(this).addClass('disabled').attr('rel','tooltip');}});$('form').submit(function(){return true;});generate_zone_option();});