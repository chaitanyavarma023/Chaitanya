...
catch(e){return false;}}});var View=Class.extend({__name__:'LuCI.view',__init__:function(){var vp=document.getElementById('view');DOM.content(vp,E('div',{'class':'spinning'},_('Loading view…')));return Promise.resolve(this.load()).then(LuCI.prototype.bind(this.render,this)).then(LuCI.prototype.bind(function(nodes){var vp=document.getElementById('view');DOM.content(vp,nodes);DOM.append(vp,this.addFooter());},this)).catch(LuCI.prototype.error);},load:function(){},render:function(){},handleSave:function(ev){var tasks=[];document.getElementById('main-content').querySelectorAll('.cbi-map').forEach(function(map){tasks.push(DOM.callClassMethod(map,'save'));});return Promise.all(tasks);},handleSaveApply:function(ev,mode){return this.handleSave(ev).then(function(){classes.ui.changes.apply(mode=='0');});},handleReset:function(ev){var tasks=[];document.getElementById('main-content').querySelectorAll('.cbi-map').forEach(function(map){tasks.push(DOM.callClassMethod(map,'reset'));});return Promise.all(tasks);},addFooter:function(){var footer=E([]);if(this.handleSaveApply||this.handleSave||this.handleReset){footer.appendChild(E('div',{'class':'cbi-page-actions control-group'},[this.handleSaveApply?E('button',{'class':'btn btn-info btn-small cbi-button-apply important','click':classes.ui.createHandlerFn(this,'handleSaveApply')},[_('Save & Apply')]):'',' ',this.handleSave?E('button',{'class':'btn btn-info btn-small cbi-button-save','click':classes.ui.createHandlerFn(this,'handleSave')},[_('Save')]):'',' ',this.handleReset?E('button',{'class':'btn btn-info btn-small cbi-button-reset','click':classes.ui.createHandlerFn(this,'handleReset')},[_('Reset')]):'']));}
...
DOM.content(document.querySelector('#main-content'),E('pre',{'class':'alert-message error'},e.message));e.reported=true;}
