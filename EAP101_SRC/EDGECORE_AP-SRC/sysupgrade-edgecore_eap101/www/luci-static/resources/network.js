...
return null;}
return bits;}
function initNetworkState(refresh){if(_state==null||refresh){_init=_init||Promise.all([L.resolveDefault(callNetworkInterfaceDump(),[]),L.resolveDefault(callLuciBoardJSON(),{}),L.resolveDefault(callLuciNetworkDevices(),{}),L.resolveDefault(callLuciWirelessDevices(),{}),L.resolveDefault(callLuciHostHints(),{}),getProtocolHandlers(),L.resolveDefault(uci.load('network')),L.resolveDefault(uci.load('wireless')),L.resolveDefault(uci.load('luci')),L.resolveDefault(callLuciECDhcpList(),{})]).then(function(data){var netifd_ifaces=data[0],board_json=data[1],luci_devs=data[2];var s={isTunnel:{},isBridge:{},isSwitch:{},isWifi:{},ifaces:netifd_ifaces,radios:data[3],hosts:data[4],netdevs:{},bridges:{},switches:{},hostapd:{},EChosts:data[9]};for(var name in luci_devs){var dev=luci_devs[name];if(isVirtualIfname(name))
...
return'None';var ciphers=Array.isArray(enc.ciphers)?enc.ciphers.map(function(c){return c.toUpperCase()}):['NONE'];if(Array.isArray(enc.wpa)){var versions=[],suites_out=[],suites=Array.isArray(enc.authentication)?enc.authentication.map(function(a){return a.toUpperCase()}):['NONE'];for(var i=0;i<enc.wpa.length;i++)
switch(enc.wpa[i]){case 1:versions.push('WPA');break;default:versions.push('WPA%d'.format(enc.wpa[i]));break;}
for(var j=0;j<suites.length;j++)
switch(suites[j]){case"802.1X":suites_out.push('EAP');break;case"SAE":suites_out.push('Personal');break;default:suites_out.push(suites[j]);break;}
if(versions.length>1)
return'mixed %s %s (%s)'.format(versions.join('/'),suites_out.join(', '),ciphers.join('/'));return'%s %s (%s)'.format(versions[0],suites_out.join(', '),ciphers.join('/'));}
return'Unknown';}
function formatWifiEncryption_new(enc){var encr_out;switch(enc){case"none":encr_out="None";break;case"psk+ccmp":encr_out="WPA-PSK (CCMP)";break;case"psk+tkip+ccmp":encr_out="WPA-PSK (TKIP, CCMP)";break;case"psk2+ccmp":encr_out="WPA2-PSK (CCMP)";break;case"psk2+tkip+ccmp":encr_out="WPA2-PSK (TKIP, CCMP)";break;case"wpa+ccmp":encr_out="WPA-EAP (CCMP)";break;case"wpa+tkip+ccmp":encr_out="WPA-EAP (TKIP, CCMP)";break;case"wpa2+ccmp":encr_out="WPA2-EAP (CCMP)";break;case"wpa2+tkip+ccmp":encr_out="WPA2-EAP (TKIP, CCMP)";break;case"sae":encr_out="WPA3 Personal";break;case"sae-mixed":encr_out="WPA3 Personal Transition";break;case"wpa3":encr_out="WPA3 Enterprise";break;case"wpa3-mixed":encr_out="WPA3 Enterprise Transition";break;default:encr_out="None";break;}
return'%s'.format(encr_out);}
...
return this.instantiateWifiNetwork(sid||netname,radioname,radiostate,netid,netstate,netstate?_state.hostapd[netstate.ifname]:null);},getIfnameOf:function(obj){return ifnameOf(obj);},getDSLModemType:function(){return initNetworkState().then(function(){return _state.hasDSLModem?_state.hasDSLModem.type:null;});},getHostHints:function(){return initNetworkState().then(function(){return new Hosts(_state.hosts);});},getECDhcpList:function(){return initNetworkState().then(function(){return new Hosts(_state.EChosts);});}});Hosts=baseclass.extend({__init__:function(hosts){this.hosts=hosts;},getHostnameByMACAddr:function(mac){return this.hosts[mac]?this.hosts[mac].name:null;},getIPAddrByMACAddr:function(mac){return this.hosts[mac]?this.hosts[mac].ipv4:null;},getIP6AddrByMACAddr:function(mac){return this.hosts[mac]?this.hosts[mac].ipv6:null;},getHostnameByIPAddr:function(ipaddr){for(var mac in this.hosts)
...
rv.push('%s'.format(addrs[i].address));return rv;},getNetmask:function(){var addrs=this._ubus('ipv4-address');if(Array.isArray(addrs)&&addrs.length)
