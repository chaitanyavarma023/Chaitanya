...
nand_restore_certificate() {
	sync
	local ubidev=$( nand_find_ubi $CI_UBIPART )
	local ubivol="$( nand_find_volume $ubidev certificates )"
	[ ! "$ubivol" ] && {
		echo "cannot find certificates volume."
		return 1
	}
	mkdir /tmp/new_cert
	if ! mount -t ubifs /dev/$ubivol /tmp/new_cert; then
		echo "mounting ubifs $ubivol failed"
		rmdir /tmp/new_cert
		return 1
	fi

	tar xz -f "$1" -C /tmp/new_cert 2>/dev/null
	umount /tmp/new_cert
	sync
	rmdir /tmp/new_cert
}
...
nand_upgrade_prepare_ubi() {
	...
	local mtdnum2="$( find_mtd_index "$CI_UBIPART_B" )"
	...
	if [ ! "$mtdnum2" ]; then
		echo "cannot find ubi mtd partition $CI_UBIPART_B"
		return 1
	fi

	[ "$CI_IPQ807X" = 1 ] && {
		if [ $CI_DUALBOOT -eq 1 ]; then
			ubidetach -f -m $mtdnum2
		else
			ubidetach -f -m $mtdnum
		fi
	}
	...
	[ "$cert_ubivol" ] && ubirmvol /dev/$ubidev -N certificates || true
	...
	# create certificates for non-ubifs rootfs
	if [ "$rootfs_type" != "ubifs" ]; then
		if ! ubimkvol /dev/$ubidev -N certificates -S 20; then
			echo "cannot initialize certificates volume"
			return 1
		fi
	fi
...
nand_do_upgrade_success() {
	...
	local cert_tar="/tmp/tip-cert.tgz"
	sync
	[ -f "$conf_tar" ] && nand_restore_config "$conf_tar"
	[ -f "$cert_tar" ] && nand_restore_certificate "$cert_tar"
...
nand_upgrade_tar() {
	...
	[ "$has_rootfs" = "1" ] && {
		...
		sleep 1
		echo "Setting active partition to $CI_NEWACTIVE"
		fw_setenv active $CI_NEWACTIVE
	}
	nand_do_upgrade_success
}
