...
drv_mac80211_init_device_config() {
	...
	config_add_boolean noscan ht_coex acs_exclude_dfs prc_disabled
	...
	config_add_boolean \
		...
		he_su_beamformee \
		he_mu_beamformer \
		he_dl_muofdma_disable \
		he_ul_muofdma_disable \
		he_ul_mumimo \
		vht_txop_ps \
		htc_vht \
		rx_antenna_pattern \
...
drv_mac80211_init_iface_config() {
	...
	config_add_string limit_up_enable limit_up limit_down_enable limit_down
...
# for TWT (from qsdk spf11.1)
mac80211_add_he_capabilities_twt() {
	local __out= oifs
...
mac80211_hostapd_setup_base() {
	...
	json_get_vars noscan ht_coex prc_disabled
	...
	if [ "$enable_ax" != "0" ]; then
		...
		[ -n "$he_ul_mumimo" ] && append base_cfg "he_ul_mumimo=$he_ul_mumimo" "$N"
		[ -n "$he_dl_muofdma_disable" ] && append base_cfg "he_dl_muofdma_disable=$he_dl_muofdma_disable" "$N"
		[ -n "$he_ul_muofdma_disable" ] && append base_cfg "he_ul_muofdma_disable=$he_ul_muofdma_disable" "$N"
...
mac80211_generate_mac() {
	...
	case $board in
	edgecore,eap101|edgecore,eap102|edgecore,oap103|edgecore,eap104)
...
mac80211_iw_interface_add() {
	...
	[ "$rc" = 0 ] && {
		[ -e "/sys/class/net/$ifname" ] && ip link set dev "$ifname" down >/dev/null 2>&1
	}
...
mac80211_prepare_vif() {
	...
	case "$mode" in
		...
		sta)
			...
			if [ -z "$wds" -o "$wds" = "0" ] && [ -e /usr/sbin/ebtables ]; then
				if [ -z "$has_arpnat" ]; then
					ebtables -t nat -N ARPNAT_I && ebtables -t nat -A PREROUTING -j ARPNAT_I
					ebtables -t nat -N ARPNAT_O && ebtables -t nat -A POSTROUTING -j ARPNAT_O
					echo "600 0 1 0" > /proc/net/arpnat
					has_arpnat=1
				fi 2>/dev/null
				ebtables -t nat -A ARPNAT_I --in-interface "$ifname" -j arpnat --arpnat-target ACCEPT
				ebtables -t nat -A ARPNAT_O --out-interface "$ifname" -j arpnat --arpnat-target ACCEPT
			fi
		;;
	esac
...
mac80211_vap_cleanup() {
	...
	if [ "$service" = "wpa_supplicant" -a -e /usr/sbin/ebtables ]; then
		ebtables -t nat -F ARPNAT_I
		ebtables -t nat -F ARPNAT_O
	fi 2>/dev/null
}
...
drv_mac80211_setup() {
	...
	[ -n "$country" ] && {
		iw reg get | grep -q "^country $country:" || {
			...
			if [ "$country_idx" == "1" ]; then
				iw reg set "US"
			elif [ "$country_idx" == "2" ]; then
				iw reg set "DE"
			else
				iw reg set "$country"
			fi
	...
	if [ "$(uci get wireless.wmesh.device)" == "nil" ]; then
		iw dev mesh0 del
	fi
	...
	mac80211_prepare_iw_htmode
	has_arpnat=
	for_each_interface "sta adhoc mesh monitor" mac80211_prepare_vif ${multiple_bssid}
	...
	[ -n "$hostapd_ctrl" ] && {
		...
		if [ "$no_reload" != "0" ]; then
			...
			while [ $ubus_retry -lt 3 ]; do
				hostapd_res="$(ubus call hostapd config_add "{\"iface\":\"$primary_ap\", \"config\":\"${hostapd_conf_file}\"}")"
				err1=$?
				[ "$err1" = 0 -a ! -z "$hostapd_res" ] && break
				ubus_retry=$(($ubus_retry + 1))
				sleep 2
			done

			[ "$err1" != 0 -o -z "$hostapd_res" ] && {
