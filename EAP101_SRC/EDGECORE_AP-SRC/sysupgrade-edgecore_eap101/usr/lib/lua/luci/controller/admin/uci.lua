...
function index()
    entry({"admin", "uci", "api", "check"}, call("action_api_dummy")).leaf = true 
end
...
function action_api_dummy()
    local uci = luci.model.uci.cursor()
    wan_ip = uci:get("network", "wan", "ipaddr") or ""
    if wan_ip ~= "" then
        luci.http.write('{ "status": "1", "wan_ip": "' .. wan_ip .. '"}');
    else
