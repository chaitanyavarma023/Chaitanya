...
module( "luci.acn.ip", package.seeall )
...
local function __bless(x)
	return setmetatable( x, {
		__index	= luci.acn.ip.cidr,
		__add   = luci.acn.ip.cidr.add,
		__sub   = luci.acn.ip.cidr.sub,
		__lt	= luci.acn.ip.cidr.lower,
		__eq	= luci.acn.ip.cidr.equal,
		__le	=
			function(...)
				return luci.acn.ip.cidr.equal(...) or luci.acn.ip.cidr.lower(...)
			end
	} )
end
...
-- @return			luci.acn.ip.cidr instance or nil if given address was invalid
-- @see				IPv6
-- @see				Hex
...
--- Transform given hex-encoded value to luci.acn.ip.cidr instance of specified
-- address family.
-- @param hex		String containing hex encoded value
-- @param prefix	Prefix length of CIDR instance (optional, default is 32/128)
-- @param family	Address family, either luci.acn.ip.FAMILY_INET4 or FAMILY_INET6
-- @param swap		Bool indicating whether to swap byteorder on low endian host
-- @return			luci.acn.ip.cidr instance or nil if given value was invalid
-- @see				IPv4
-- @see				IPv6
...
-- @param addr	A luci.acn.ip.cidr instance to compare against
