...
function hostname_str(val, min, max)
	min = tonumber(min)
	max = tonumber(max)

	if val ~= nil and min ~= nil and max ~= nil then
		return (minlength(val, min) and maxlength(val, max) and (val:match("^[a-zA-Z0-9-]+$")))
	end
	return false
end
...
function custom_dns(val)
	if val ~= nil then
		local tmp_arr = util.split(val, " ")
		if #tmp_arr > 4 then
			return false
		else
			for i = 1, #tmp_arr do
				return ip.IPv4(i) and true or false
			end
		end
		-- return ip.IPv4(val) and true or false
	end
...
function limited_len_str(val, min, max)
	val = tostring(val)
	min = tonumber(min)
	max = tonumber(max)
