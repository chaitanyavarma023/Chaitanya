			...
			then
				...
				for netidx, netstate in ipairs(radiostate.interfaces) do
					if type(netstate) == "table" then
						...
						if not netstate.ifname then
							local netconfig = netstate.config
							if netconfig then
								config_ifname = netconfig.ifname
							end
							if config_ifname then
								netstate.ifname = config_ifname
							end
						end

						if type(netstate.ifname) == "string" and
	...
	for n, i in ipairs(nxo.getifaddrs()) do
		...
		if _tunnel[name] or not (_iface_ignore(name) or _iface_virtual(name)) then
			...
			if i.family == "packet" then
				...
				_interfaces[name].macaddr = i.addr and ipc.checkmac(i.addr)
...
function get_wifiphy(self, phy)
	if utl.exec("iw ".. phy .." info") then
		return wifiphy(phy)
	end
end
...
function interface._ubus_iwinfo(self, field)
	if not _ubusdevcache[self.ifname] then
		_ubusdevcache[self.ifname] = utl.ubus("iwinfo", "info", { device = self.ifname }) or {}
	end
...
function interface.acn_bridge_ports(self)
	local bridge_name = _uci:get("network", "bridge", "name")

	if bridge_name then
		local stat = utl.ubus("network.device", "status", { name = bridge_name })

		if type(stat) == "table" and type(stat["bridge-vlans"]) == "table" then
			for _, v in ipairs(stat["bridge-vlans"]) do
				if type(v) == "table" and type(v.ports) == "table" and self.ifname:gsub(bridge_name .. ".", "") == tostring(v.id) then
					local iface
					local ifaces = { }

					for _, iface in ipairs(v.ports) do
...
function interface.acn_ports(self)

	local bridge =  _bridge[self.ifname]
	if bridge then
		local iface
		local ifaces = { }
		for _, iface in ipairs(bridge.ifnames) do

			-- XXX TODO - clean this up
			-- eth1 is only used for its mac
      -- TODO remove it after finished migratrion from eth0.X to ethX
			--if iface.name ~= "eth1" then
      ifaces[#ifaces+1] = iface.name
			--end
		end
		return ifaces
	end
...
function interface.wif_is_up(self) -- for wireless interfaces only
	local wif_ssid = self:_ubus_iwinfo("ssid")

	if wif_ssid == nil then
		up = (self:type() == "alias")
	else
		up = true
	end
...
function wifiphy.__init__(self, name)
	self.iwinfo = _wifi_iwinfo_by_ifname(name, true)
end
...
function wifidev.get_wifinets(self)
	...
	_uci:foreach("wireless", "wifi-iface",
		function(s)
			...
			if s.device == self.sid and s.created == "1" then
...
function wifinet.is_up(self)
	...
	return (ifc and ifc:wif_is_up() or false)
end
...
function wifinet.remote_mac(self)
	local remote_mac
	if 	self.iwinfo.mode == "Client" then
		for r_mac, _ in pairs(self.iwinfo.assoclist) do
			remote_mac = r_mac
		end
	end
	return remote_mac
