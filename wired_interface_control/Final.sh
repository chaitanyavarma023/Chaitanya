...
# Function to setup DNAT forwarding
setup_forwarding_DNAT() {
  local interface=$1
  local proto=$2
  local port=$3

  printf "Enter destination IP address for %s port %s: " "$proto" "$port" >&2
  read -r dest_ip </dev/tty

  printf "Enter destination port (default: %s): " "$port" >&2
  read -r dport </dev/tty
  dport=${dport:-$port}  # Use default if empty

  # Add DNAT rule
  iptables -t nat -I PREROUTING -i "$interface" -p "$proto" --dport "$port" -j DNAT --to-destination "${dest_ip}:${dport}"
  # Allow forwarded traffic (corrected line)
  iptables -I FORWARD -i "$interface" -p "$proto" -d "$dest_ip" --dport "$dport" -j ACCEPT
}

# Function to setup SNAT
setup_forwarding_SNAT() {
  local interface=$1
  local proto=$2
  local port=$3

  printf "Enter source IP address to SNAT to for %s port %s: " "$proto" "$port" >&2
  read -r src_ip </dev/tty
